...
import { ArrowLeft, ArrowDown, Monitor, Search, Edit2, Plus, X, Upload, Database, Activity, CheckCircle2, AlertCircle, ExternalLink } from "lucide-react";
                  ...
                  >
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 gap-3">
                      ...
                      {onViewTicket && (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-accent bg-accent/10 px-3 py-1.5 rounded-lg border border-accent/20 hover:bg-accent hover:text-white transition-all shadow-sm"
                        >
                    ...
                    <div className="pt-2 pb-2 space-y-4">
                      ...
                      {(() => {
                        ...
                        // Logical weighting to resolve "Same Minute" jumping
                          ...
                          <>
                            {allEvents.map((ev, i) => (
                              <React.Fragment key={i}>
                                ...
                                {ev.type === 'ict_action' && (
                                  <div className="relative pl-5 ml-2 border-l-2 border-border/60 pb-2">
                                    <div className="absolute -left-[5px] top-1.5 w-2 h-2 bg-orange-500 rounded-full ring-[4px] ring-bg"></div>
                                    ...
                                    <p className="text-xs text-ink-muted font-medium leading-relaxed max-w-3xl mt-1">{ev.text as string}</p>
                                ...
                                {ev.type === 'problem_report' && (
                                  <div className="relative pl-5 ml-2 border-l-2 border-border/60 pb-2">
                                    <div className="absolute -left-[5px] top-1.5 w-2 h-2 bg-red-500 rounded-full ring-[4px] ring-bg"></div>
                                    <p className="text-xs font-bold text-red-500 mb-2">Problem Still Exists (Reported by Dept)</p>
                                    <div className="text-xs text-ink-muted bg-red-500/5 border border-red-500/10 p-3 rounded-lg max-w-3xl leading-relaxed">
                                      ...
                                      {ev.details && <><br/><span className="font-semibold text-ink mt-1 block">Details:</span> {ev.details as string}</>}
                                    </div>
                                    {ev.dateStr && <p className="text-[10px] text-ink-muted font-medium mt-2">{ev.dateStr}</p>}
                                ...
                                {ev.type === 'referral' && (
                                  <div className="relative pl-5 ml-2 border-l-2 border-border/60 pb-2">
                                    <div className="absolute -left-[5px] top-1.5 w-2 h-2 bg-purple-500 rounded-full ring-[4px] ring-bg"></div>
                                    <p className="text-[11px] font-bold text-purple-500 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                                      <AlertCircle className="w-3.5 h-3.5" /> Assessed & Referred Externally
                                    </p>
                                    <div className="text-xs text-ink-muted bg-purple-500/5 border border-purple-500/10 p-3 rounded-lg max-w-3xl leading-relaxed">
                                      <span className="font-semibold text-ink">Provider:</span> {ev.provider as string}
                                      <br/>
                                      <span className="font-semibold text-ink mt-1 block">Reason:</span> {ev.reason as string}
                                    </div>
                                    {ev.dateStr && <p className="text-[10px] text-ink-muted font-medium mt-2">{ev.dateStr}</p>}
                                  </div>
                                )}
