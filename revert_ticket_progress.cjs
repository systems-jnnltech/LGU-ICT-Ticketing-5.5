...
const updatedProgress = `<div className="flex justify-between relative max-w-4xl mx-auto px-4 overflow-x-auto pb-4">
                  {(() => {
                    ...
                    if (hasExternal) {
                      steps.push({ label: 'Referred', key: 'REFERRED TO EXTERNAL TECHNICIAN' });
                      ...
                      return (
                        <div key={index} className="flex flex-col items-center relative flex-1 min-w-[80px]">
                          ...
                          {(isCompleted || isCurrent) && (() => {
                            ...
                            if (timestampStr) {
                                return <span className="text-[9px] text-slate-400 mt-1 text-center font-medium">{format(new Date(timestampStr), 'MMM d, h:mm a')}</span>;
                            }
                            return null;
                          })()}
                        </div>
...
const originalProgress = `<div className="flex justify-between relative max-w-4xl mx-auto px-4">
                  {[
...
content = content.replace(updatedProgress, originalProgress);

const actionsRegex = /\{ticket\.status === 'ESCALATED TO ICT HEAD' && \([\s\S]*?\)\}/m;
